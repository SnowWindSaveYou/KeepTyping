...
const TopicInfo = (props) => (
  <div>
  <SubBlock className="TopicInfo" style={{background:global.theme.base_color}}>

    <div className="top inner" style={{ background: global.theme.primary_color }}>
      <CircleBlock className="topic_head" radiu="35"
        style={{background:global.theme.base_color}}>
        <img style={{ width: "100%", height: "100%" }}
          src={props.topic.avater ? props.topic.avater : defaultHead}></img>
      </CircleBlock>
    </div>

    <div className="middle inner">
      <p className="topic_title" style={{color:global.theme.primary_color}}>{props.topic.title}</p>
      <p className="topic_description">{props.topic.description}</p>

      <div className="follow_info pair_row">
      <div className="pair_part">
          {props.topic.member_num}<br />
          Members
          </div >
        <div className="pair_part">
          {props.topic.post_num}<br />
          Posts
          </div >
      </div>
    ...
    <div className="topic_managers">
        ...
        <ul>
          {props.topic.managers.map((manager)=>{
              return(
                <li key={manager._id} style={{color:global.theme.secondary_color}}>{manager.name}</li>
              )
            })}
        </ul>
      </div>
  ...
  <SubBlock>
    <div className="topic_tags" style={{background:global.theme.base_color,padding:"10px"}}>
      {props.topic.tags.map((tag)=>{
        return(
          <li key={tag} style={{color:global.theme.secondary_color}}>{tag}</li>
        )
      })}
    </div>
  </SubBlock>
