...
import MyPage from "./page_templates/my_page";
...
class App extends Component {
  ...
  /**
   *  Change status
   *  Child elemnets can change highest level leyout status.
   */
  ...
  componentDidMount() {
    ...
    global.followTopic = (topic, follow) => {
      if (follow) {
        this.state.my_info.topics.push(topic);
      } else {
        var i = this.state.my_info.topics.indexOf(topic);
        this.state.my_info.topics.splice(i, 1);
      }
      this.setState({});
    };
  }
      ...
      <LoginContext.Provider value={this.state}>
        <BrowserRouter basename="/">
          <Switch>
            ...
            <Route exact path="/m" component={MyPage} />
