...
router.get("/getPosts/:topicName", (req, res) => {
  TopicContoller.getPosts(req.params.topicName, req.query.page, (err, data) => {
    if (err) {
      res.json({
        success: false,
        message: err
...
router.post("/followTopic", (req, res) => {
  tokenMsg = Token.checkToken(req.headers["token"]);
  if (tokenMsg.success) {
    TopicContoller.followTopic(req.body.topic, tokenMsg.data.user_id, err => {
      if (err) {
        res.json({
          success: false,
