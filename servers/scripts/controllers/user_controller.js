...
/**
 * Control request about user.
 */
var UserController = {
  // Get user over view information
  ...
  followUser(toUserId, userId, callback) {
    UserModel.findByIdAndUpdate(
      toUserId,
      { $push: { follower: userId }, $inc: { follower_num: 1 } },
      (err, res) => {
        if (!err) {
  ...
  unfollowUser(toUserId, userId, callback) {
    UserModel.findByIdAndUpdate(
      toUserId,
      { $pull: { follower: userId }, $inc: { follower_num: -1 } },
      (err, res) => {
        if (!err) {
          UserModel.findByIdAndUpdate(userId, {
